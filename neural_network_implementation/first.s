...
__main FUNCTION
	 ...
	 MOV R0,#0;					Case input for LOGIC_AND
	 BL printp;	
	 BL __LOGIC_AND;
	 
	 MOV R0,#1;					Case input for LOGIC_OR
	 BL printp;
	 BL __LOGIC_OR;	
	 
	 MOV R0,#2;					Case input for LOGIC_NAND
	 BL printp;	
	 BL __LOGIC_NAND;
	 
	 MOV R0,#3;					Case input for LOGIC_NOR
	 BL printp;	
	 BL __LOGIC_NOR;
	 
	 MOV R0,#4;					Case input for LOGIC_XOR
	 BL printp;	
	 BL __LOGIC_XOR;
	 
	 MOV R0,#5;					Case input for LOGIC_XNOR
	 BL printp;	
	 BL __LOGIC_XNOR;
	 
	 MOV R0,#6;					Case input for LOGIC_NOT
	 BL printp;	
	 BL __LOGIC_NOT;
...
__exponential FUNCTION
     ...
     VLDR.F32 S11,=1;			Storing e^x
     VLDR.F32 S12,=1;			Temp variable by previous term     
	 
LOOP1 
	 CMP R12,R11;			Compare count
     BLE LOOP2;				If count is less than no. of terms enter LOOP1
	 POP {LR};	
     BX lr;						else STOP
	 
LOOP2  
	 VMUL.F32 S12,S12,S10; 		
     VMOV.F32 S13,R12;			Move the count in R9 to S13
     VCVT.F32.S32 S13,S13; 		Convert into signed 32bit number
     VDIV.F32 S12,S12,S13;		Divide temp_var
     VADD.F32 S11,S11,S12;		Add temp_var
...
__val2 FUNCTION
	 ...
	 VLDR.F32 S14,= 0.5;		Store 0.5
	 VCMP.F32 S9,S14;			Compare		
	 VMRS    APSR_nzcv, FPSCR;	 
	 MOV R0, R4;
	 MOV R1, R5;
	 MOV R2, R6;				
	 MOVGT	R1, #1;				If Y > 0.5 CASE
	 MOVLT	R1, #0;				If Y < 0.5 CASE
	 POP {LR};
	 BX lr;
	 ENDFUNC	
